...
async function generateJWT(options) {
  const { SignJWT } = await import("./esm-D4V7BTDB.mjs");
...
async function loadSuperJSON() {
  const superjson = await import("./dist-XGXNJLOO.mjs");
...
function createSchemaTask(params) {
  ...
  const task = {
    ...
    trigger: async (payload, options, requestOptions) => {
      ...
      return await trigger_internal(taskMetadata2 && taskMetadata2.exportName ? `${taskMetadata2.exportName}.trigger()` : `trigger()`, params.id, payload, parsePayload, {
        queue: customQueue,
        ...options
      }, requestOptions);
    },
    batchTrigger: async (items, options, requestOptions) => {
      const taskMetadata2 = taskCatalog.getTaskManifest(params.id);
      return await batchTrigger_internal(taskMetadata2 && taskMetadata2.exportName ? `${taskMetadata2.exportName}.batchTrigger()` : `batchTrigger()`, params.id, items, options, parsePayload, requestOptions, customQueue);
    },
    triggerAndWait: (payload, options) => {
      const taskMetadata2 = taskCatalog.getTaskManifest(params.id);
      return new TaskRunPromise((resolve, reject) => {
        triggerAndWait_internal(taskMetadata2 && taskMetadata2.exportName ? `${taskMetadata2.exportName}.triggerAndWait()` : `triggerAndWait()`, params.id, payload, parsePayload, {
          queue: customQueue,
          ...options
        }).then((result) => {
          resolve(result);
    ...
    batchTriggerAndWait: async (items, options) => {
      const taskMetadata2 = taskCatalog.getTaskManifest(params.id);
      return await batchTriggerAndWait_internal(taskMetadata2 && taskMetadata2.exportName ? `${taskMetadata2.exportName}.batchTriggerAndWait()` : `batchTriggerAndWait()`, params.id, items, parsePayload, options, void 0, customQueue);
    }
  };
...
@google-cloud/precise-date/build/src/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-PEGTRPBK.mjs.map
